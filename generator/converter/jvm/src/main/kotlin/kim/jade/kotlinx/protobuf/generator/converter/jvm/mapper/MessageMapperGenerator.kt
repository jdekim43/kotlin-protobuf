...
import kim.jade.kotlinx.protobuf.generator.converter.jvm.mapper.util.extention.delegatorNameEscaped
...
class MessageMapperGenerator : MapperGenerator<Descriptors.Descriptor> {
    ...
    private fun Descriptors.FieldDescriptor.getCodeForToKotlin(
        ...
        U_LONG, U_INT -> "$variableName.%M" to arrayOf(
            *variableNameArguments,
            MemberName("kim.jade.kotlinx.protobuf.util", "asKotlinType"),
        )
    ...
    private fun Descriptors.OneofDescriptor.getCodeForToKotlin(): Pair<String, Array<Any>> {
        ...
        code.append("\t)[obj.%N.number]?.invoke()")
    ...
    private fun Descriptors.Descriptor.writeToKotlinConvertFunctionTo(
        ...
        for (field in realFields) {
            ...
            if (field.isNullable) {
                // A proto3 `optional` maps to a nullable Kotlin property, so an absent value has to come
                // back as null rather than as the protobuf default. The reverse direction already skips
                // the setter when the property is null; without this the two disagree and a round trip
                // turns null into 0.
                val accessorName = field.outputVariableNameString
                    .toPascalCase(ProtobufWordSplitter)
                    .delegatorNameEscaped

                toKotlinTypeFunction.addCode(
                    "\t%N = if (obj.has%L()) $code else null,\n",
                    *arrayOf(field.outputVariableNameString, accessorName, *arguments),
                )
            } else {
                toKotlinTypeFunction.addCode(
                    "\t%N = $code,\n",
    ...
    private fun Descriptors.FieldDescriptor.getCodeForToProtobuf(
        ...
        U_LONG, U_INT -> "$variableName.%M" to arrayOf(
            *variableNameArguments,
            MemberName("kim.jade.kotlinx.protobuf.util", "asJavaType"),
        )
    ...
    private fun Descriptors.Descriptor.writeToProtobufConvertFunctionTo(
        ...
        for (field in realFields) {
            ...
            if (field.isNullable) {
                ...
                toKotlinTypeFunction.addStatement(
                    ...
                    field.protoAccessorName.toPascalCase(ProtobufWordSplitter).delegatorNameEscaped,
                    *arguments
                )
                ...
                toKotlinTypeFunction.addStatement(
                    "builder.$function($code)",
                    *arrayOf(field.protoAccessorName.toPascalCase(ProtobufWordSplitter).delegatorNameEscaped, *arguments)
                )
            }
        }
        for (oneOf in realOneofs) {
            ...
            toKotlinTypeFunction.addStatement("null -> Unit")
            toKotlinTypeFunction.endControlFlow()
        }
